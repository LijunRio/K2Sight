...
def main():
    ...
    config_path = "/u/home/lj0/Code/florence2/Detection_Project/conf/experiments/experiment_2.yaml"
    ...
    if config['model']['peft']['use_peft']:
        ## Load exist peft model checkpoint if have :)
        if config['model']['peft']['lora_checkpoint'] not in [None, "False"]:
            ...
            # use the config to initialize the model
    ...
    # else finetune entire language part, only freeze the vision part
    elif config['model']['finetune']:
        for param in model.vision_tower.parameters():
            param.requires_grad = False
    else: # entire model trainable
        for param in model.parameters():
            param.requires_grad = True
